"""Connection lifecycle for Diligent ESG Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from diligent_esg_client import DiligentEsgClient
...
_SECRET = "diligent_esg_connections"
...
@chat.function(
    "connect_diligent_esg",
    "Connect Diligent ESG account via credentials.",
    action_type="write",
    chain_callable=True,
    event="diligent-esg-connector.connect_diligent_esg",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_diligent_esg(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Diligent ESG Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "Diligent ESG Account",
...
@chat.function(
    "list_connections",
    "List connected Diligent ESG accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_diligent_esg",
    "Disconnect Diligent ESG account.",
    action_type="write",
    chain_callable=True,
    event="diligent-esg-connector.disconnect_diligent_esg",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_diligent_esg(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
